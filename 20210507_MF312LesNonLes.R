...
L.data<-Read10X("./data/MF312_resequenced/thick") 
NL.data<-Read10X("./data/MF312_resequenced/P82_posttreatment")
...
L_clone<-read.csv("./data/MF312_resequenced/thick/TCR/filtered_contig_annotations.csv")
NL_clone<-read.csv("./data/MF312_resequenced/P82_posttreatment/TCR/filtered_contig_annotations.csv")
...
clono_L <- read.csv("./data/MF312_resequenced/thick/TCR/clonotypes.csv")
clono_NL <- read.csv("./data/MF312_resequenced/P82_posttreatment/TCR/clonotypes.csv")
...
head(NonLes$cdr3s_aa, 200)

sum(grepl(pattern ="TRA:CALMDSSYKLIF;TRB:CASSVANRGTYEQYF;TRB:CASSLAGLYTYTQYF",Les$cdr3s_aa)) #659
sum(grepl(pattern ="TRA:CALMDSSYKLIF;TRB:CASSVANRGTYEQYF;TRB:CASSLAGLYTDTQYF",NonLes$cdr3s_aa)) #27
...
Les <- AddMetaData(Les, "MF312",  col.name= "patient")
NonLes <- AddMetaData(NonLes, "MF312", col.name= "patient")

Les <- AddMetaData(Les, "MF312_lesional",  col.name= "sample")
NonLes <- AddMetaData(NonLes, "MF312_posttreatment", col.name= "sample")
...
# QC plots
pdf("./output/QC/VlnPlot_MF312_Les.pdf",
    width=14, height=7)
VlnPlot(Les, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

pdf("./output/QC/VlnPlot_MF312_PostTreat.pdf",
    width=14, height=7)
...
Les2 = as.Seurat(Les.sce2) #8592 (was 8871) cells
NonLes2 = as.Seurat(NonLes.sce2) #12988 (was 13678) cells



#compute 2 SD up and down from mean of total mRNAs (nCount_RNA)
upper_bound <- 10^(mean(log10((Les)$nCount_RNA)) +  #13922
                     2*sd(log10((Les)$nCount_RNA)))
lower_bound <- 10^(mean(log10((Les)$nCount_RNA)) -
                     2*sd(log10((Les)$nCount_RNA))) #516
...
upper_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) +  #3822
                     2*sd(log10((Les2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) -
                     2*sd(log10((Les2)$nFeature_RNA))) #340

upper_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) +  #5744
                     2*sd(log10((NonLes2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) -
                     2*sd(log10((NonLes2)$nFeature_RNA))) #283
...
NonLes3 <- subset(NonLes2, subset = nFeature_RNA > 250 & nFeature_RNA < 6000 & percent.mt<12)

##how many genes per cell after filtering
median(Les$nFeature_RNA)  #1206
median(Les3$nFeature_RNA)  #1287

median(NonLes$nFeature_RNA)  #1459
median(NonLes3$nFeature_RNA)  #1667

#Save
saveRDS(Les3, "./data/MF312_Lesional_filtered.RDS")
saveRDS(NonLes3, "./data/MF312_PostTreatment_filtered.RDS")
