...
L.data<-Read10X("../downloads/Pat65_MF_lesion_nolesion/lesional/filtered_feature_bc_matrix") 
NL.data<-Read10X("../downloads/Pat65_MF_lesion_nolesion/nonlesional/filtered_feature_bc_matrix")
...
L_clone<-read.csv("../downloads/Pat65_MF_lesion_nolesion/lesional/TCR/filtered_contig_annotations.csv")
NL_clone<-read.csv("../downloads/Pat65_MF_lesion_nolesion/nonlesional/TCR/filtered_contig_annotations.csv")
...
clono_L <- read.csv("../downloads/Pat65_MF_lesion_nolesion/lesional/TCR/clonotypes.csv")
clono_NL <- read.csv("../downloads/Pat65_MF_lesion_nolesion/nonlesional/TCR/clonotypes.csv")
...
sum(grepl(pattern ="CAGSSGSARQLTF",Les$cdr3s_aa)) #847
sum(grepl(pattern ="CAGSSGSARQLTF",NonLes$cdr3s_aa)) #948
...
Les <- AddMetaData(Les, "65",  col.name= "patient")
NonLes <- AddMetaData(NonLes, "65", col.name= "patient")

Les <- AddMetaData(Les, "Pat65_lesional",  col.name= "sample")
NonLes <- AddMetaData(NonLes, "Pat65_nonlesional", col.name= "sample")
...
# QC plots
pdf("./output/QC/VlnPlot_Pat65_Les.pdf",
    width=14, height=7)
VlnPlot(Les, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

pdf("./output/QC/VlnPlot_Pat65_NonLes.pdf",
    width=14, height=7)
...
VlnPlot(Les, features = c("nFeature_RNA", "nCount_RNA", "percent.mt","percent.ribo"), ncol = 4)
VlnPlot(NonLes, features = c("nFeature_RNA", "nCount_RNA", "percent.mt","percent.ribo"), ncol = 4)
...
#Les.sce2$DoubletScore<-doub_score
...
NonLes.scr=NonLes.scr[,doub_score1<thresh1] 
...
Les2 = as.Seurat(Les.sce2) #13965(was 14219) cells
NonLes2 = as.Seurat(NonLes.scr) #12409(was 12556) cells



#compute 2 SD up and down from mean of total mRNAs (nCount_RNA)
upper_bound <- 10^(mean(log10((Les)$nCount_RNA)) +  #13113
                     2*sd(log10((Les)$nCount_RNA)))
lower_bound <- 10^(mean(log10((Les)$nCount_RNA)) -
                     2*sd(log10((Les)$nCount_RNA))) #351
...
#nFeature
#compute 2 SD up and down from mean of total mRNAs (nCount_RNA)
upper_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) +  #3411
                     2*sd(log10((Les2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) -
                     2*sd(log10((Les2)$nFeature_RNA))) #246

upper_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) +  #3734
                     2*sd(log10((NonLes2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) -
                     2*sd(log10((NonLes2)$nFeature_RNA))) #261
...
VlnPlot(subset(NonLes2, percent.mt<40), features = "percent.mt") #12
...
median(Les$nFeature_RNA)  #951
median(Les3$nFeature_RNA)  #1091

median(NonLes$nFeature_RNA)  #1066
median(NonLes3$nFeature_RNA)  #1180

#Save
saveRDS(Les3, "./data/Pat65_Lesional_filtered.RDS")
saveRDS(NonLes3, "./data/Pat65_NonLesional_filtered.RDS")
