...
L.data<-Read10X("./data/MF311_resequenced/thick") 
NL.data<-Read10X("./data/MF311_resequenced/nonlesional")
...
L_clone<-read.csv("./data/MF311_resequenced/thick/TCR/filtered_contig_annotations.csv")
NL_clone<-read.csv("./data/MF311_resequenced/nonlesional/TCR/filtered_contig_annotations.csv")
...
clono_L <- read.csv("./data/MF311_resequenced/thick/TCR/clonotypes.csv")
clono_NL <- read.csv("./data/MF311_resequenced/nonlesional/TCR/clonotypes.csv")
...
sum(grepl(pattern ="TRA:CAGKTSYDKVIF;TRB:CASSFGGVSPLHF",Les$cdr3s_aa)) #3360
sum(grepl(pattern ="TRB:CASSFGGVSPLHF",NonLes$cdr3s_aa)) #both: 1, only A:5, only B:2
...
Les <- AddMetaData(Les, "MF311",  col.name= "patient")
NonLes <- AddMetaData(NonLes, "MF311", col.name= "patient")

Les <- AddMetaData(Les, "MF311_lesional",  col.name= "sample")
NonLes <- AddMetaData(NonLes, "MF311_nonlesional", col.name= "sample")
...
# QC plots
pdf("./output/QC/VlnPlot_MF311_Les.pdf",
    width=14, height=7)
VlnPlot(Les, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

pdf("./output/QC/VlnPlot_MF311_NonLes.pdf",
    width=14, height=7)
...
Les2 = as.Seurat(Les.sce2) #16776 (was 16817) cells
NonLes2 = as.Seurat(NonLes.sce2) #7226 (was 7226) cells



#compute 2 SD up and down from mean of total mRNAs (nCount_RNA)
upper_bound <- 10^(mean(log10((Les)$nCount_RNA)) +  #6315
                     2*sd(log10((Les)$nCount_RNA)))
lower_bound <- 10^(mean(log10((Les)$nCount_RNA)) -
                     2*sd(log10((Les)$nCount_RNA))) #489
...
upper_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) +  #2101
                     2*sd(log10((Les2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) -
                     2*sd(log10((Les2)$nFeature_RNA))) #336

upper_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) +  #2841
                     2*sd(log10((NonLes2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) -
                     2*sd(log10((NonLes2)$nFeature_RNA))) #560
...
plot2 <- FeatureScatter(Les2, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") #3500 nfeature
...
Les3 <- subset(Les2, subset = nFeature_RNA > 250 & nFeature_RNA < 3500 & percent.mt<12)
NonLes3 <- subset(NonLes2, subset = nFeature_RNA > 250 & nFeature_RNA < 4000 & percent.mt<12)

##how many genes per cell after filtering
median(Les$nFeature_RNA)  #843
median(Les3$nFeature_RNA)  #864

median(NonLes$nFeature_RNA)  #1269
median(NonLes3$nFeature_RNA)  #1296

#Save
saveRDS(Les3, "./data/MF311_Lesional_filtered.RDS")
saveRDS(NonLes3, "./data/MF311_NonLesional_filtered.RDS")
