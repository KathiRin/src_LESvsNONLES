...
L.data<-Read10X("./data/P84_resequenced/lesional") 
NL.data<-Read10X("./data/P84_resequenced/posttreatment")
...
L_clone<-read.csv("./data/P84_resequenced/lesional/TCR/filtered_contig_annotations.csv")
NL_clone<-read.csv("./data/P84_resequenced/posttreatment/TCR/filtered_contig_annotations.csv")
...
clono_L <- read.csv("./data/P84_resequenced/lesional/TCR/clonotypes.csv")
clono_NL <- read.csv("./data/P84_resequenced/posttreatment/TCR/clonotypes.csv")
...
sum(grepl(pattern ="TRA:CALSDPGTASKLTF;TRB:CASKYYRGANYGYTF",Les$cdr3s_aa)) #1490
sum(grepl(pattern ="CASKYYRGANYGYTF",NonLes$cdr3s_aa)) #0
...
NonLes <- AddMetaData(NonLes, "Posttreatment", col.name= "tissue")

Les <- AddMetaData(Les, "Pat84",  col.name= "patient")
NonLes <- AddMetaData(NonLes, "Pat84", col.name= "patient")

Les <- AddMetaData(Les, "Pat84_lesional",  col.name= "sample")
NonLes <- AddMetaData(NonLes, "Pat84_posttreatment", col.name= "sample")
...
# QC plots
pdf("./output/QC/VlnPlot_Pat84_Les.pdf",
    width=14, height=7)
VlnPlot(Les, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

pdf("./output/QC/VlnPlot_Pat84_PostTreat.pdf",
    width=14, height=7)
...
sum(exclude_doublet)# cells (doublets) to exclude =279
...
sum(exclude_doublet1)# cells (doublets) to exclude =690
...
Les2 = as.Seurat(Les.sce2) #12288 (was 12400) cells
NonLes2 = as.Seurat(NonLes.sce2) #15676 (was 16107) cells



#compute 2 SD up and down from mean of total mRNAs (nCount_RNA)
upper_bound <- 10^(mean(log10((Les)$nCount_RNA)) +  #19395
                     2*sd(log10((Les)$nCount_RNA)))
lower_bound <- 10^(mean(log10((Les)$nCount_RNA)) -
                     2*sd(log10((Les)$nCount_RNA))) #506
...
upper_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) +  #4508
                     2*sd(log10((Les2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) -
                     2*sd(log10((Les2)$nFeature_RNA))) #327

upper_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) +  #1584
                     2*sd(log10((NonLes2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) -
                     2*sd(log10((NonLes2)$nFeature_RNA))) #252
...
plot2 <- FeatureScatter(Les2, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") #5000 nfeature
...
plot2 <- FeatureScatter(NonLes2, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")#2000 nf
...
Les3 <- subset(Les2, subset = nFeature_RNA > 250 & nFeature_RNA < 5000 & percent.mt<12)
               
NonLes3 <- subset(NonLes2, subset = nFeature_RNA > 250 & nFeature_RNA < 2500 &  percent.mt<12)

##how many genes per cell after filtering
median(Les$nFeature_RNA)  #1168
median(Les3$nFeature_RNA)  #1246

median(NonLes$nFeature_RNA)  #604
median(NonLes3$nFeature_RNA)  #622

#Save
saveRDS(Les3, "./data/Pat84_Lesional_filtered.RDS")
saveRDS(NonLes3, "./data/Pat84_PostTreatment_filtered.RDS")
