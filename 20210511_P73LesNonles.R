...
L.data<-Read10X("./data/P73_resequenced/lesional") 
NL.data<-Read10X("./data/P73_resequenced/nonlesional")
...
L_clone<-read.csv("./data/P73_resequenced/lesional/TCR/filtered_contig_annotations.csv")
NL_clone<-read.csv("./data/P73_resequenced/nonlesional/TCR/filtered_contig_annotations.csv")
...
clono_L <- read.csv("./data/P73_resequenced/lesional/TCR/clonotypes.csv")
clono_NL <- read.csv("./data/P73_resequenced/nonlesional/TCR/clonotypes.csv")
...
head(Les$cdr3s_aa, 200)
tail(NonLes$cdr3s_aa, 20)
head(clono_L)
head(clono_NL)

#search for top clone (clonotype 1)
sum(grepl(pattern ="TRA:CALSGYGNKLVF;TRB:CASRDRGTDTQYF",Les$cdr3s_aa)) #204
sum(grepl(pattern ="TRA:CALSGYGNKLVF;TRB:CASRDRGTDTQYF",NonLes$cdr3s_aa)) #8
...
Les <- AddMetaData(Les, "Pat73",  col.name= "patient")
NonLes <- AddMetaData(NonLes, "Pat73", col.name= "patient")

Les <- AddMetaData(Les, "Pat73_lesional",  col.name= "sample")
NonLes <- AddMetaData(NonLes, "Pat73_nonlesional", col.name= "sample")
...
# QC plots
pdf("./output/QC/VlnPlot_Pat73_Les.pdf",
    width=14, height=7)
VlnPlot(Les, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
dev.off()

pdf("./output/QC/VlnPlot_Pat73_NonLes.pdf",
    width=14, height=7)
...
sum(exclude_doublet)# cells (doublets) to exclude =149
...
sum(exclude_doublet1)# cells (doublets) to exclude =195
...
Les2 = as.Seurat(Les.sce2) #22899 (was 23048) cells
#NonLes2 = as.Seurat(NonLes.sce2) 
NonLes3 = as.Seurat(NonLes.scr) #to see which cells were marked as doublets 

doubletsNL<-subset(NonLes3, subset=DoubletScore>thresh1 )
doubletsNLbig<-subset(NonLes3, subset=DoubletScore>4 )

FeatureScatter(doubletsNL, feature1 = "nFeature_RNA", feature2 = "percent.mt")
FeatureScatter(doubletsNL, feature1 = "nFeature_RNA", feature2 = "nCount_RNA")

FeatureScatter(NonLes, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
FeatureScatter(doubletsNL, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
FeatureScatter(doubletsNLbig, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")


#FAZIT: do not exclude doublets in NonLes according to doublet threshold, too many non-doublet cells
NonLes2<-NonLes3
rm(NonLes3)

#compute 2 SD up and down from mean of total mRNAs (nCount_RNA)
upper_bound <- 10^(mean(log10((Les)$nCount_RNA)) +  #6788
                     2*sd(log10((Les)$nCount_RNA)))
lower_bound <- 10^(mean(log10((Les)$nCount_RNA)) -
                     2*sd(log10((Les)$nCount_RNA))) #587
...
upper_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) +  #2098
                     2*sd(log10((Les2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) -
                     2*sd(log10((Les2)$nFeature_RNA))) #380

upper_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) +  #7000
                     2*sd(log10((NonLes2)$nFeature_RNA)))
lower_bound <- 10^(mean(log10((NonLes2)$nFeature_RNA)) -
                     2*sd(log10((NonLes2)$nFeature_RNA))) #225
...
VlnPlot(subset(Les2, nFeature_RNA<1200), features = "nFeature_RNA", pt.size = 0.5) #350
...
plot2 <- FeatureScatter(Les2, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") #4000 nfeature
...
FeatureScatter(NonLes2, feature1 = "percent.ribo", feature2 = "DoubletScore")
FeatureScatter(NonLes2, feature1 = "nCount_RNA", feature2 = "DoubletScore")



#filter each sample seperately
Les3 <- subset(Les2, subset = nFeature_RNA > 350 & nFeature_RNA < 4000 & percent.mt<12)
               
NonLes3 <- subset(NonLes2, subset = nFeature_RNA > 250 & nFeature_RNA < 4000 &  percent.mt<12)

##how many genes per cell after filtering
median(Les$nFeature_RNA)  #807
median(Les3$nFeature_RNA)  #821

median(NonLes$nFeature_RNA)  #1493
median(NonLes3$nFeature_RNA)  #1632

#Save
saveRDS(Les3, "./data/Pat73_Lesional_filtered.RDS")
saveRDS(NonLes3, "./data/Pat73_Nonlesional_filtered.RDS")
