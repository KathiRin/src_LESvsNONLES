...
L.data<-Read10X("./data/P65_resequenced/lesional") 
NL.data<-Read10X("./data/P65_resequenced/nonlesional")
...
L_clone<-read.csv("./data/P65_resequenced/lesional/TCR/filtered_contig_annotations.csv")
NL_clone<-read.csv("./data/P65_resequenced/nonlesional/TCR/filtered_contig_annotations.csv")
...
clono_L <- read.csv("./data/P65_resequenced/lesional/TCR/clonotypes.csv")
clono_NL <- read.csv("./data/P65_resequenced/nonlesional/TCR/clonotypes.csv")
...
sum(grepl(pattern ="TRA:CAGSSGSARQLTF;TRB:CASSQDAWDAPTGELFF",Les$cdr3s_aa)) #839
sum(grepl(pattern ="TRA:CAGSSGSARQLTF;TRB:CASSQDAWDAPTGELFF",NonLes$cdr3s_aa)) #933
...
Les <- AddMetaData(Les, "Pat65",  col.name= "patient")
NonLes <- AddMetaData(NonLes, "Pat65", col.name= "patient")
...
sum(exclude_doublet)# cells (doublets) to exclude =240
...
sum(exclude_doublet1)# cells (doublets) to exclude =140
...
upper_bound <- 10^(mean(log10((Les)$nCount_RNA)) +  #17665
                     2*sd(log10((Les)$nCount_RNA)))
lower_bound <- 10^(mean(log10((Les)$nCount_RNA)) -
                     2*sd(log10((Les)$nCount_RNA))) #362
...
upper_bound <- 10^(mean(log10((Les2)$nFeature_RNA)) +  #4201
...
Les3 <- subset(Les2, subset = nFeature_RNA > 250 & nFeature_RNA < 4200 & percent.mt<12)
               
NonLes3 <- subset(NonLes2, subset = nFeature_RNA > 250 & nFeature_RNA < 4000 &  percent.mt<12)

##how many genes per cell after filtering
median(Les$nFeature_RNA)  #1077
median(Les3$nFeature_RNA)  #1271
